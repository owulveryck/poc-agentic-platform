...
> Solves one problem: steering agents that cannot call the validation server from
...
1. Install the binaries (`make install` from the repo — puts `ppg`,
   ...
   explicitly if you started the validation server on a different port.
...
3. From there, Copilot's black-box planning is steered by the same invariants
   the validation server serves to every other agent.
