...
| Term | Definition |
|---|---|
| **PPG** | The project/brand name — a historical acronym (*Platform Planning Gateway*) kept in binary names, HTTP surface and env vars (`ppg`, `ppg-guard`, `PPG_URL`, …). Per [ADR-130](../decisions/ADR-130-gateway-naming.md) the component it originally named is now the *validation server*; the acronym survives as a proper noun. |
| **Policy corpus** | Everything the validation server evaluates as one unit: ADR-paired `.rego` files, built-in rules, operator and session skill companions, the skill-governance policies, and the catalog-ranking policy. Loaded at startup and rebuilt atomically on `SIGHUP` (fail-safe: a broken reload keeps the previous corpus serving). |
...
| **Escalation** | The intended behavior on contradiction: a hard block that surfaces enough information for a human to fix the underlying policies, feeding the monotonic improvement loop. *Implemented coverage today* is the deterministic **livelock** escalation (`409 POLICY_CONFLICT` after 3 rejections with an identical violation set, consecutive or not): the conflict gets a stable `conflict_id`, blocks every session producing that set, survives restarts, and is closed only by `ppg escalations resolve` after the corpus fix. General contradiction detection is undecidable and not claimed — a plan whose every submission trips a *different* set never escalates. See the known-limits entry in [design decisions](design-decisions-and-limits.md) and the how-to [Resolve a policy conflict](../how-to/resolve-a-policy-conflict.md). |
