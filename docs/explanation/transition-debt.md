...
PoC ships with 2 compensatory artifacts out of 8 total (ratio = 0.25,
under the 0.3 `DEBT_ALERT` threshold, so the report reads `health: "OK"`);
each amplifier ADR added (ADR-090, ADR-100, ADR-110) dilutes the ratio
further below the alert line. Removing two amplifiers — or adding a third
compensatory artifact — flips it back to `DEBT_ALERT`, which is exactly the
