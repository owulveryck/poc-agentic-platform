...
func main() {
	...
	// The guard is a short-lived process: one journal open per invocation is
	// fine (appends are flock-serialized across processes). A nil writer
	// (telemetry disabled) is valid and inert.
	jw := journal.Open(root, "ppg-guard", projectDir)
	...
	if in.HookEventName == "SessionStart" {
		if err := recordSession(in, st, st); err != nil {
			...
			jw.Emit(journal.Event{
				...
				Attrs:     map[string]any{"agent": "claude-code"},
			})
		}
...
// write tool that passed (code "ok"), one ppg.guard.block per denial. Blocks
// carry the machine reason_code and — unless payload capture is off — the
// model-facing message as "reply"; a fail-closed infra block is ERROR, a
// policy denial WARN. Ungated calls (non-write tools, harness metadata) emit
// nothing. The tool_input content never enters the journal.
func emitVerdict(jw *journal.Writer, in hookInput, ss store.SessionStore, block bool, code, msg string) {
	...
	ev := journal.Event{
		...
		Attrs: map[string]any{
			"tool": in.ToolName,
			"path": relativeTarget(targetPath(in), in.CWD),
		},
	}
	if !block {
...
// an artifact verifier, it returns whether to block, a machine-readable verdict
// code (a journal.Reason* constant on block, verdictAllow/verdictNone on
// allow), and the semantic message for the model. It gates on the tool name
// (not merely the presence of a path), checks path scope and session binding
// locally, then verifies the edited content against the artifact-view policy
// through verify. A nil verifier skips the content step (used by offline tests).
func decide(payload []byte, rawTicket string, verify artifactVerifier) (bool, string, string) {
	var in hookInput
	if err := json.Unmarshal(payload, &in); err != nil {
		return true, journal.ReasonGuardError, "PPG_GUARD_ERROR: unreadable hook payload; blocking (fail-closed)."
	}
	if !isWriteTool(in.ToolName) {
		return false, verdictNone, "" // not a file-mutating tool: nothing to guard
	}
